...
class TitanicMLP(nn.Module):
    def __init__(self, input_dim, hidden_units=(64, 32), dropout_rate=0.5, output_activation='sigmoid'):
        super().__init__()
        layers = []
        prev = input_dim
...
def train_pytorch(
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch+1}/{num_epochs} - Loss: {epoch_loss:.4f} - MAE: {epoch_mae:.4f} - "
...
class MLOpsTitanicPipeline:
    ...
    def train(
        ...
        if log_to_mlflow:
            mlflow.start_run()
        try:
            model, metrics, signature, input_example = train_strategy(df, **strategy_params)
            if log_to_mlflow:
                for k, v in metrics.items():
                    if isinstance(v, (int, float)):
                        mlflow.log_metric(k, v)
                if model_name and hasattr(mlflow, 'sklearn') and "RandomForest" in model_name:
    ...
    def run(self, train_file: str, strategies: Dict[str, Tuple[Callable, Dict[str, Any]]]):
...
def pytorch_mlp_strategy(df: pd.DataFrame, **kwargs):
    ...
    history = train_pytorch(
        model, optimizer, kwargs.get("mlp_params", {}).get("output_activation", "sigmoid"),
        train_loader, val_loader, kwargs.get("num_epochs", 20), device, patience=kwargs.get("patience", 5)
    )
    # Validation metrics
    model.eval()
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    strategies = {
        ...
        "PyTorchMLP": (
            ...
            {"mlp_params": {"hidden_units": (64, 32), "dropout_rate": 0.5, "output_activation": "sigmoid"},
             "lr": 1e-3,
             "num_epochs": 30,
             "batch_size": 32,
             "device": "cpu"}
